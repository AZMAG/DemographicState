        ...
        if (!isInited) {
            ...
            tp.subscribe("gfxLayer-loaded", function () {
                ...
                function AddBufferedGraphic(e) {
                    if (e.graphic) {
                        let bufferGraphicsLayer = mapsutils.map.findLayerById(
                            "bufferGraphics"
                        );
                ...
                function ProcessSelection(gfx) {
                    reportsutils
                        .GetData(
                            config.layerDef["blockGroups"],
                            null,
                            gfx.geometry
                        )
                        .then(function (data) {
                            var acsData = utilities.summarizeFeatures(
                                data.acsData.features
                            );
                            var censusData = utilities.summarizeFeatures(
                                data.censusData.features
                            );
                            ...
                            if (data.acsData.features.length === 0) {
                                    ...
                                    features: [
                                        {
                                            ...
                                            count:
                                                data.censusData.features.length,
                                ...
                                tp.publish(
                                    "open-report-window",
                                    reportsutils.selectedReport,
                                    "acs"
                                );
                                $customGeographyReports.hide();
                                utilities.AddHighlightGraphics(
                                    data.acsData.features,
                                    $useZoom.is(":checked")
                                );
                                $(".reportFormArea").hide();
