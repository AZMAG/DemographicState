...
require(["dojo/topic", "esri/tasks/QueryTask"], function (tp, QueryTask) {
    ...
    function SetupBlockGroupGrid(blockGroups) {
        ...
        let columns = fields
            .filter(({ universeField }) => universeField !== 2)
            .map(({ fieldName, tableHeader, fieldType }) => {
                return {
                    field: fieldName,
                    title: tableHeader.includes("Minority")
                        ? "Minority"
                        : tableHeader,
                    format: fieldType
                        ? `{0:${fieldType[0].toLowerCase()}0}`
                        : undefined,
                    width: 150,
                };
            });
    ...
    function SetupTitle6Grid(attr) {
        ...
        let dataSrc = [
            ...
            },
            {
                ...
                PercentOfBlocks:
                    attr["affected_income_below_poverty_c"] / totalBlockCount,
