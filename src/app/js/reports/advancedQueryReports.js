...
require(["dojo/topic", "esri/tasks/QueryTask"], function (tp, QueryTask) {
    ...
    function InitAdvancedQuery() {
        ...
        function submitQuery() {
            if (resultCount === 0) {
                alert(
                    "0 block groups meet your current criteria.  Please refine your query and try again."
                );
        ...
        function RunQuery() {
            ...
            qt.execute(q).then(function (res) {
                ...
                app.GetData(app.config.layerDef["blockGroups"], geoids).then(
                    function (data) {
                            ...
                            features: [
                                {
                                    ...
                                    ids: data.acsData.features.map(
                                        (feature) => feature.attributes["GEOID"]
                                    ),
                                },
                            ],
        ...
        function BuildQueryString() {
            ...
            for (var i = 0; i < QueryItems.length; i++) {
                ...
                if (
                    QueryItems[i].type === "number" ||
                    QueryItems[i].type === "percent"
                ) {
                    var operator = $(QueryItems[i].operator).val();
                    var inputBoxes = $(
                        QueryItems[i].id +
                            " input.style1.k-formatted-value.k-input"
                    );
                    var min = 0;
                    ...
                    if (inputBoxes.length > 1) {
                        ...
                        if (inputBoxes[0]) {
                            if (inputBoxes[0].value) {
                                inputValue = inputBoxes[0].value.replace(
                                    /,/g,
                                    ""
                                );
            ...
            QueryItems.forEach(function (queryItem) {
                if (
                    queryItem.normalizeField &&
                    !normalizedFieldsLU[queryItem.normalizeField]
                ) {
        ...
        function OnChange(e) {
            ...
            if (dataItem.Sign !== "between") {
                $(
                    '<input class="style1" placeholder="value"></input>'
                ).insertAfter(item);
            } else {
                $(
        ...
        function AddRow(dataItem) {
            ...
            if (dataItem.Type !== undefined) {
                if (count > 1) {
                    target.append(
                        "<select class='joinDDLClass' id='joinDDL" +
                            count +
                            "'></select>"
                    );
