...
function GetPartyLetter(party) {
    ...
    if (party.indexOf("Democratic") > -1) {
        ...
        return `<strong title="${party}" style="font-weight: bold; color: green;"> (${party.charAt(
            0
        )})</strong>`;
    }
}
...
function GetChannelsHTML(channels) {
    ...
    channels.forEach(function (channel) {
        ...
        if (type) {
            channelsHTML += `<a class="socialChannel" title="Visit the ${
                channel.type
            } page" target="_blank" href="${type.url + channel.id}"><i class="${
                type.icon
            }"></i></a>`;
        }
    });
    ...
    if (rep) {
        return `<div class="repContainer">
                    <div class="repPicContainer">
                        ${
                            rep.photoUrl
                                ? `<img title="${rep.name}" class="rep-pic" src="${rep.photoUrl}" alt="${rep.name}">`
                                : ""
                        }
                    </div>
                    <div class="repInfoContainer">
                        <div><i class="fas fa-user-alt"></i> <strong>${
                            rep.name
                        }</strong>${GetPartyLetter(rep.party)} - ${
                ...
                app.config.googleCivicOffices.forEach(function (conf) {
                    if (office.name.indexOf(conf.name) > -1) {
                        if (data.officials && office.officialIndices) {
                            for (
                                let i = 0;
                                i < office.officialIndices.length;
                                i++
                            ) {
                                let rep =
                                    data.officials[office.officialIndices[i]];
...
app.PopupFormat = async function (gfx) {
    ...
    return `
                ...
                <hr class="pop">
                <div>Total Population: <strong>${attr[
                    "TOTAL_POP"
                ].toLocaleString()}</strong></div>
                <div>Minority Population: <strong>${attr[
                    "MINORITY_POP"
                ].toLocaleString()}</strong></div>
                ${
                    attr["MEDIAN_AGE"]
                        ? `<div>Median Age: <strong>${attr["MEDIAN_AGE"]} years</strong></div>`
                        : ""
                }
                <div>Number of Households: <strong>${attr[
                    "TOTAL_HOUSEHOLDS"
                ].toLocaleString()}</strong></div>
                ${
                    attr["MEDIAN_HOUSEHOLD_INCOME"]
                        ? `<div>Median Household Income: <strong>$${attr[
