...
define([
    ...
    "dojo/domReady!",
], function (colorRampConfig, config, tp, QueryTask) {
    ...
    var mapsutils = {
        ...
        GetRampsByNumAndType: function (type, numBreaks) {
            let classBreakSet =
                colorRampConfig[type]["ClassBreakSets"][numBreaks];
        ...
        GetCurrentMapsParams: async function () {
            ...
            if ($dynamicCBRCheckbox.is(":checked") && classType !== "Custom") {
                breaks = await this.GetDynamicClassBreaks(
                    cbrCount,
                    classType,
                    conf
                );
            ...
            //Get color ramp info
            let rampKey =
                $colorRamp.find(".cRamp").data("id") || config.DefaultColorRamp;
            let type =
                $colorRamp.find(".cRamp").data("type") ||
        ...
        view: null,
        map: null,
        GetDynamicClassBreaks: async function (cbrCount, classType, conf) {
            let q = {
                returnGeometry: false,
                outFields: conf.NormalizeField
                    ? [conf.FieldName, conf.NormalizeField]
                    : [conf.FieldName],
            ...
            return qt.execute(q).then(function (res) {
                ...
                res.features.forEach((feature) => {
                    if (conf.NormalizeField) {
                        arr.push(
                            feature.attributes[conf.FieldName] /
                                feature.attributes[conf.NormalizeField] || 0
                        );
                    } else {
                ...
                console.log({ cbrCount, classType, conf, breakValues });
                return breakValues;
            });
        },
    };
