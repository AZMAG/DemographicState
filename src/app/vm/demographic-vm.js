		...
		function(
			...
			var DemographicVM = new function() {
				...
				self.init = function() {
					...
					var chartWindow = $('#demographicView')
						.kendoWindow({
					...
					helpButton.click(function() {
						...
						} else if (tab[0].textContent === 'ACS 2016 Data') {
				...
				self.interactiveSelectionQueryHandler = function(results) {
					...
					self.reportConfigItem = demographicConfig.reports.blockGroups;
					...
					var tab = tabStrip.select();
					if (tab.length > 0) {
						if (tab[0].textContent === 'ACS 2016 Charts' || tab[0].textContent === 'Census 2010 Charts') {
							self.reloadChart();
						}
					}
					...
					if (tab[0]) {
						if (self.reportType === 'cog') {
							...
							if (
								...
								$('#demSource').html(
									"Source: United States Census Bureau, American Community Survey 2012-2016 5yr Estimates <br> (Interpolation method used, <a class='interHelp link'>Click here</a> for more details) "
								);
							}
						} else {
							//Sets the correct source label at bottom of report
						...
						if (tab[0].textContent === 'Census 2010 Charts' || tab[0].textContent === 'ACS 2016 Charts') {
							self.reloadChart();
						}
					}
				};
				...
				self.getData = function() {
					...
					layerDelegate.query(
						censusUrl,
						self.censusDataQueryHandler,
						self.dataQueryFault,
						null,
						whereClause,
						true
					);
					layerDelegate.query(acsUrl, self.acsDataQueryHandler, self.dataQueryFault, null, whereClause, true);
				};
					...
					$.each(aggValues, function(index, item) {
						...
						if (item.chartCategory !== '') {
							if (item.chartCategory in self.aggValuesCensusGroupedByChartCategory) {
								self.aggValuesCensusGroupedByChartCategory[item.chartCategory].push(
								...
								self.aggValuesCensusGroupedByChartCategory[item.chartCategory] = [
									aggValues[item.fieldName]
								];
							}
						}
				...
				self.useCompareClicked = function(sender) {
					...
					var url = self.reportConfigItem.compareCensusUrl || self.reportConfigItem.censusRestUrl;
					...
					if (sender.target.id === 'demACSUseComp') {
						...
						type = 'ACS';
						url = self.reportConfigItem.compareACSUrl || self.reportConfigItem.ACSRestUrl;
					}
					if (compareComboBoxObj === undefined || compareComboBoxObj === null) {
						// Get the place names
						var whereClause = self.reportConfigItem.compareWhereClause;
				...
				self.addCompareValues = function(type) {
					...
					$.each(dataSource, function(index, item) {
						...
						if (
							item.derivedTargetField !== undefined &&
							item.derivedTargetField !== null &&
							item.derivedTargetField !== ''
						) {
							...
							if (item.derivedPercentOfField !== '' && item.derivedPercentOfField !== undefined) {
								...
								if (
									item.comparePercentValue !== null &&
									!isNaN(item.comparePercentValue) &&
									item.comparePercentValue !== Infinity
								) {
									item.comparePercentValueFormatted =
										magNumberFormatter.formatValue(item.comparePercentValue) + '%';
								} else {
									item.comparePercentValueFormatted = '-';
								}
							}
							...
							if (
								item.derivedDensityAreaField !== undefined &&
								item.derivedDensityAreaField !== null &&
								item.derivedDensityAreaField !== ''
							) {
				...
				self.onChartListSelectionChanged = function() {
					...
					self.groupedItems =
						self.aggValuesCensusGroupedByChartCategory[self.selectedCategoryObj[0].childNodes[1].innerHTML];

					if (this.element[0].id.indexOf('Census') === -1) {
						self.groupedItems =
							self.aggValuesACSGroupedByChartCategory[
								self.selectedCategoryObj[0].childNodes[1].innerHTML
							];
					}
				...
				self.createChart = function(type, animation) {
					...
					var templateString =
						"#= category #<br>#= kendo.format('{0:P}', percentage) #<br>#= kendo.format('{0:N0}', value) #";
				...
				self.exportToExcel = function(sender) {
					...
					if (exportButtonId === 'demACSExportResults') {
						...
						headerValue = self.communityName + ' ACS 2012-2016 ACS 5-Year Estimates';
					...
					if (
						self.compareFeature !== null &&
						exportButtonId !== 'demCensusExportSelFeatResults' &&
										...
										$.each(acsFieldsConfig.fields, function(j, el) {
											if (el.tableHeader === row.cells[1].value) {
												switch (el.indentLevel) {
													case 1:
														row.cells[1].formula = row.cells[1].value.replace(
															el.tableHeader,
															'="   ' + el.tableHeader + '"'
														);
														break;
