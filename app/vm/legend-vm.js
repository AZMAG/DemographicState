...
(function () {
    ...
    define([
        ...
        function (dc, tp, mapModel, conf, helpView, helpVM, legendview, magNumberFormatter, bookmarkDelegate) {
            ...
            var LegendVM = new function () {
                ...
                self.init = function (initializationData) {
                    ...
                    tp.subscribe("legendStateO", function (event) { self.openWindow(); });
                    tp.subscribe("legendStateC", function (event) { self.closeWindow(); });
                ...
                self.addTOCLayers = function (layersToAdd) {//add layer options here...
                    if (mapModel.mapInstance.id === mapModel.baseMapInstance.id) {
                        ...
                        $.each(layersToAdd, function (index, layer) {
                            ...
                            dc.create("input", { id: chkId, type: "checkbox", class: "vertMiddle", onclick: self.onCheckBoxClick, checked: layer.visible }, liId, "first"); //TODO legendLayerOptions check is about here on visible
                ...
                self.mapLoaded = function (map) {
                    if (map.id === mapModel.baseMapInstance.id) {
                        if (!self.legendInitialized) {
                            ...
                            self.legend.startup();
                            document.getElementById("legendTitle").innerHTML = self.legendMapTitle;
                            document.getElementById("title2").innerHTML = self.legendMapTitle;
                            document.getElementById("dataSource").innerHTML = self.sourceInfo;
